...
function Title(props){
  ...
  return(
    <View className="h-full flex flex-col bg-[var(--white-color)] rounded-[20] items-center justify-center w-fit pt-[1] pb-[1] pl-[2] pr-[2] sticky top-[200px] mt-[50%] drop-shadow-textBox" style={styles.titleView}>
      <Text className="font-[GeoBold] text-3xl drop-shadow-text align-center">
...
function Cards(props){
  ...
  return (
        <View className="h-full flex flex-col w-fit relative">
          {cardList.map((card, i) => {
          return(
            ...
            <motion.div className="flex flex-col items-center justify-center relative pt-[20px] mt-0 mr-[8vw] mb-[4vh] ml-[8vw] w-[112%] h-full overflow-visible" key={i} viewport={{ once: true, amount: 0.8 }}>
              <View className="absolute overflow-visible bottom-0 left-1 right-1 top-0 mt-[55px] w-full h-[75vh]" style={styles.splash}>
                <Text className="fadedTitle">{module}</Text>
              </View>
              ...
              whileInView="visible"
              >
                <Text className="font-[LeagueSpartan] align-center text-2xl" style={styles.cardTitle}>{card.title}</Text>
                <Image className="w-[18vmin] h-auto rounded-[20] mb-[15]" source={card.image} alt={card.title}></Image>
                <Link href={card.link} className="group"><Text className="font-[LeagueSpartan] text-xl group-hover:text-2xl -mb-[5] text-[var(--black-color) group-hover:text-[var(--dark-color)] no-underline">Click here to view the app yourself!</Text></Link>
                <Link href={card.git} className="group"><Text className="font-[LeagueSpartan] text-xl group-hover:text-2xl -mb-[5] text-[var(--black-color) group-hover:text-[var(--dark-color)] no-underline">Or view the code on GitHub</Text></Link>
                <Text className="font-[LeagueSpartan] text-xl -mb-[20]">Skills/Languages used:</Text>
                <FlatList
                data={card.skills.map((skill, i) => {
                  ...
                  renderItem={({item}) => <Text className="font-[Poppins] text-lg">{item.key}</Text>}/>
              </motion.div>
            </motion.div>
          </motion.div>)
          })}
